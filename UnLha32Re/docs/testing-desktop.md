...
`test.ps1 -Profile Release` / `release.ps1` から呼ぶ `test-release.ps1` も同じ分離とDWM監視を使います。
短時間チェックの子プロセス全体には90秒以下の上限を設定し、時間切れでも外側のDWM監視を通常終了して結果を回収します。
正常な自己生成書庫を使う候補単独検証で、原版の表示・状態の全組合せ比較は行いません。
