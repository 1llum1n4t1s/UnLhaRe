...
cleanup()
{
    /* 圧縮中断で encode_end_st0 に到達しなかった場合も解放する。 */
    free_st0_buffer();
    if (g_infp) {
        fclose(g_infp);
        g_infp = NULL;
    ...
    if (g_outfp) {
        const int closes_temporary = fileno(g_outfp) == temporary_fd;
        fclose(g_outfp);
        g_outfp = NULL;
        if (closes_temporary) {
            temporary_fd = -1;
    ...
    if (g_update_archive_fp) {
        fclose(g_update_archive_fp);
...
lha_exit(int status)
{
    /* 局所ガードは呼び出し側が資源を所有する。通常経路の cleanup はハンドラが行う。 */
...
void lha_capture_bytes(const void* data, size_t length) {
    capture_write((const char*)data, length);
}
...
int lha_fputc(int c, FILE *f) {
    ...
    return Lha_WriteCompressionCharacter(c, f);
}
