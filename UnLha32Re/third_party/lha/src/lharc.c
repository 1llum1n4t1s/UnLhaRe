...
{
    /* This core is invoked repeatedly from a DLL process. */
    optind = 0;
    optarg = NULL;
    optopt = 0;
/* options */
...
cleaning_files(int *v_filec, char ***v_filev)
{
    ...
    for (i = 0; i < filec; i++)
        if (Lha_CompressionInputStat(filev[i], &stbuf) < 0) {
            flags[i] = 0x04;
                ...
                if ((flags[j] & 0x07) == 0x00) {    /* regular file, not
                    ...
                    if (STREQU(p, filev[j]))
                        flags[j] = 0x04;    /* delete */
                             ...
                             * deleted/ignored */
#ifdef LHA_LIBRARY
            if (Lha_HasFlatCompressionInputs()) {
                /* 展開済み入力では親子を除外せず、同じディレクトリーの重複だけを除く。 */
                for (j = i + 1; j < filec; j++)
                    if ((flags[j] & 0x07) == 0x02 && STREQU(p, filev[j])) flags[j] = 0x04;
                continue;
            }
#endif
            for (j = i + 1; j < filec; j++) {
...
need_file(char *name)
{
...
#ifdef LHA_LIBRARY
    boolean included = cmd_filec == 0 && Lha_CommandPatternMatches("*", name, FALSE);
    for (i = 0; i < cmd_filec && !included; i++)
        included = Lha_CommandPatternMatches(cmd_filev[i], name, FALSE);
    if (!included)
        return FALSE;
    for (i = 0; exclude_files && exclude_files[i]; i++)
        if (Lha_CommandPatternMatches(exclude_files[i], name, TRUE))
            return FALSE;
    return TRUE;
#else
    if (cmd_filec == 0)
    ...
    for (i = 0; i < cmd_filec; i++) {
        ...
        if (patmatch(cmd_filev[i], name, 0) ||
            (leaf && patmatch(cmd_filev[i], leaf + 1, 0)))
            return TRUE;
    }
...
/* API 入力の文字コードと、書庫の保存文字コードを混同しない。 */
boolean
need_file_header(LzHeader *header)
{
#ifdef LHA_LIBRARY
    int i;
    boolean included = cmd_filec == 0 && Lha_CommandHeaderPatternMatches("*", header, FALSE);
    for (i = 0; i < cmd_filec && !included; i++)
        included = Lha_CommandHeaderPatternMatches(cmd_filev[i], header, FALSE);
    if (!included) return FALSE;
    for (i = 0; exclude_files && exclude_files[i]; i++)
        if (Lha_CommandHeaderPatternMatches(exclude_files[i], header, TRUE)) return FALSE;
    return TRUE;
#else
    return need_file(header->name);
#endif
}
