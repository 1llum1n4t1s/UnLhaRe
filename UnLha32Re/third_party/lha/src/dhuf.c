...
static unsigned long lh2_encode_count;
/* ------------------------------------------------------------------------ */
void
start_c_dyn( /* void */ )
{
...
/* LH2 の位置木は、参照を出力する直前の入力位置に合わせて 64 バイトずつ伸ばす。 */
static void
encode_p_dyn(unsigned int pos)
{
    unsigned short bits = 0;
    unsigned char count = 0;
    int p;

    while (lh2_encode_count > nextcount) {
        make_new_node((int)(nextcount / 64));
        if ((nextcount += 64) >= (unsigned long)nn)
            nextcount = 0xffffffffUL;
    }
    for (p = s_node[N_CHAR + (pos >> 6)]; p != ROOT_P; p = parent[p]) {
        bits >>= 1;
        if (p & 1)
            bits |= 0x8000;
        if (++count == 16) {
    ...
    putcode(count, bits);
    putbits(6, (unsigned short)(pos & 0x3f));
    update_p((int)(pos >> 6));
}

void
output_dyn2(unsigned int code, unsigned int pos)
{
    encode_c_dyn(code);
    if (code >= 0x100) {
        encode_p_dyn(pos);
        lh2_encode_count += code - (256 - THRESHOLD);
    } else {
        lh2_encode_count++;
    }
}
