...
# if defined LHA_LIBRARY
FILE *Lha_OpenFile(const char *path, const char *mode);
int Lha_StatFile(const char *path, struct stat *status);
int Lha_RenameFile(const char *source, const char *destination);
int Lha_UnlinkFile(const char *path);
int Lha_MakeDirectory(const char *path);
int Lha_ChangeFileMode(const char *path, int mode);
int Lha_OpenDescriptor(const char *path, int flags, ...);
int Lha_RemoveDirectory(const char *path);
int Lha_SetFileTimes(const char *path, struct utimbuf *times);
#  ifndef __cplusplus
/* UTF-8 の API 入力は Windows のワイドファイル操作へ渡す。 */
#   define fopen(path, mode) Lha_OpenFile(path, mode)
#   define stat(path, status) Lha_StatFile(path, status)
#   define rename(source, destination) Lha_RenameFile(source, destination)
#   undef unlink
#   define unlink(path) Lha_UnlinkFile(path)
#   undef mkdir
#   define mkdir(path, mode) Lha_MakeDirectory(path)
#   undef chmod
#   define chmod(path, mode) Lha_ChangeFileMode(path, mode)
#   define open Lha_OpenDescriptor
#   define rmdir(path) Lha_RemoveDirectory(path)
#   define utime(path, times) Lha_SetFileTimes(path, times)
#  endif
# endif
#endif
...
#define LZHUFFX_METHOD          "-lhx-"
#define LZHUFFLX1_METHOD        "-lx1-"
...
#define LZHUFFX_METHOD_NUM      14
#define LZHUFFLX1_METHOD_NUM    15
/* Added N.Watazaki ..^ */
...
#define LZHUFFX_DICBIT          19      /* UNLHA32: 最大 512 KiB */
#define LZHUFFLX1_DICBIT        15      /* LX1: 32 KiB */
