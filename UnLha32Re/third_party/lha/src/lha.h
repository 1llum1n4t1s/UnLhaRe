...
extern void lha_capture_bytes(const void *data, size_t length);
#endif
...
typedef struct LzHeader {
    ...
    off_t           packed_size;
    off_t           original_size;
    unsigned short  attribute; /* 拡張ヘッダー 0x40 の 16 ビット属性を保持する。 */
    ...
    boolean         has_header_crc; /* common header CRC extension */
    unsigned char   common_header_flags; /* UNLHA32 common header extra byte */
    boolean         has_common_header_flags;
    boolean         preserve_common_header_flags; /* 書き換え時に確定した値をそのまま出力する。 */
    ...
    /* UNLHA32 Windows timestamp extension (0x41, raw FILETIME values). */
#ifdef HAVE_UINT64_T
    uint64_t        windows_creation_time;
    uint64_t        windows_last_modified_time;
    uint64_t        windows_last_access_time;
    boolean         has_windows_timestamp;
#endif

    /* UNLHA32 level-2 extensions (0x44, 0x45, 0x46). */
    unsigned int    code_page;
    boolean         has_code_page;
    /* 圧縮入力の名前は API 側の文字コード。保存する 0x46 の値とは分離する。 */
    unsigned int    input_name_code_page;
    boolean         has_input_name_code_page;
    unsigned short  unicode_name[FILENAME_LENGTH];
    size_t          unicode_name_length;
    boolean         has_unicode_name;
    unsigned short  unicode_directory[FILENAME_LENGTH];
    size_t          unicode_directory_length;
    boolean         has_unicode_directory;
}  LzHeader;
...
EXTERN FILE *g_infp;
EXTERN FILE *g_outfp;
/* 圧縮では元ファイルと旧書庫を同時に開くため、所有権を分ける。 */
EXTERN FILE *g_update_archive_fp;
