[CmdletBinding()]
param(
    [Parameter(Mandatory)][string]$TestProgram,
    [Parameter(Mandatory)][string]$Oracle,
    [Parameter(Mandatory)][string]$Candidate,
    [Parameter(Mandatory)][string]$Workspace,
    [string]$Level2Fixtures,
    [string]$ShortHeaderDirectory,
    [string]$InitialHeaderDirectory,
    [string]$MissingCrcDirectory,
    [ValidateSet('crc','short','initial','missing-crc')][string[]]$Groups=@('crc','short'),
    [ValidateSet('ascii','japanese')][string[]]$Families=@('ascii','japanese'),
    [ValidateSet(0,2)][int[]]$Methods=@(0,2),
    [ValidateSet('e','x')][string[]]$Commands=@('e','x'),
    [ValidateSet(0,1,2)][int[]]$NameModes=@(1),
    [ValidateSet(0,1)][int]$UnicodeMode=1,
    [string]$DestinationName='output',
    [string[]]$ProfileNames=@('w64','a32','w32','a64','legacy','reject','missing','rename','raw-W','raw-A-1','raw-A','raw-legacy','abort-begin','abort-missing'),
    [string[]]$FixtureNames=@()
)
$ErrorActionPreference='Stop'
$workspace=[IO.Path]::GetFullPath($Workspace)
if(Test-Path -LiteralPath $workspace){throw '既存の検証領域は上書きしません'}
if([IO.Path]::GetFileName($DestinationName) -cne $DestinationName -or $DestinationName -in '','.','..'){throw '出力先名は領域内の単一ディレクトリ名に限定します'}
$TestProgram=(Resolve-Path -LiteralPath $TestProgram).Path
$runner=(Resolve-Path -LiteralPath (Join-Path (Split-Path -Parent $TestProgram) 'DesktopRunner.exe')).Path
$oracle=(Resolve-Path -LiteralPath $Oracle).Path
$candidate=(Resolve-Path -LiteralPath $Candidate).Path
$ast=[Management.Automation.Language.Parser]::ParseFile((Join-Path $PSScriptRoot 'test-enum-state.ps1'),[ref]$null,[ref]$null)
$helper=$ast.Find({param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Invoke-EnumProbe'},$true)
if(!$helper){throw '隔離・30 秒上限付きプローブ関数が見つかりません'}
. ([scriptblock]::Create($helper.Extent.Text))
$profiles=@{
    w64=@{Api='W';Layout='w64';Selected='1';Replacement='';Pattern='*';Capacity=0}
    a32=@{Api='A';Layout='a32';Selected='1';Replacement='';Pattern='*';Capacity=0}
    w32=@{Api='W';Layout='w32';Selected='1';Replacement='';Pattern='*';Capacity=0}
    a64=@{Api='A';Layout='a64';Selected='1';Replacement='';Pattern='*';Capacity=0}
    legacy=@{Api='legacy';Layout='a64';Selected='1';Replacement='';Pattern='*';Capacity=0}
    'W-A32'=@{Api='W';Layout='a32';Selected='1';Replacement='';Pattern='*';Capacity=0}
    'W-A64'=@{Api='W';Layout='a64';Selected='1';Replacement='';Pattern='*';Capacity=0}
    reject=@{Api='W';Layout='w64';Selected='0';Replacement='';Pattern='*';Capacity=0}
    missing=@{Api='W';Layout='w64';Selected='1';Replacement='';Pattern='missing';Capacity=0}
    rename=@{Api='W';Layout='w64';Selected='1';Replacement='renamed.txt';Pattern='*';Capacity=0}
    'abort-begin'=@{Api='W';Layout='w64';Selected='1';Replacement='';Pattern='*';Capacity=0;Abort=0}
    'abort-missing'=@{Api='W';Layout='w64';Selected='1';Replacement='';Pattern='missing';Capacity=0;Abort=0}
    'raw-W'=@{Api='W';Pattern='*';Capacity=4096}
    'raw-A-1'=@{Api='A';Pattern='*';Capacity=1}
    'raw-A'=@{Api='A';Pattern='*';Capacity=4096}
    'raw-legacy'=@{Api='legacy';Pattern='*';Capacity=4096}
}
foreach($name in $ProfileNames){if(!$profiles.ContainsKey($name)){throw "未知のプローブ構成です: $name"}}
New-Item -ItemType Directory -Path (Join-Path $workspace 'fixtures') | Out-Null
$fixtures=[Collections.Generic.List[object]]::new()
function Add-ExtractionHeaderFixture([string]$Name,[string]$Path,[int]$Expected,[int]$Members){
    $fixtures.Add([pscustomobject]@{Name=$Name;Path=(Resolve-Path -LiteralPath $Path).Path;Expected=$Expected;Members=$Members})
}
if('crc' -in $Groups){
    $Level2Fixtures=(Resolve-Path -LiteralPath $Level2Fixtures).Path
    foreach($family in $Families){foreach($method in $Methods){foreach($level in 2,3){
        $root=Join-Path $Level2Fixtures "$family-jm$method"
        if($level -eq 3){$root=Join-Path $root 'level3'}
        foreach($variant in 'good','first','middle','last','all'){
            $expected=@{good=0;first=0;middle=32790;last=32790;all=32795}[$variant]
            $members=@{good=3;first=2;middle=1;last=2;all=0}[$variant]
            Add-ExtractionHeaderFixture "$family-jm$method-h$level-$variant" (Join-Path $root "$variant.lzh") $expected $members
        }
        $bytes=[IO.File]::ReadAllBytes((Join-Path $root 'good.lzh'))
        if($bytes[-1] -ne 0){throw '正常書庫の終端がありません'}
        $noEnd=Join-Path $workspace "fixtures/$family-jm$method-h$level-no-end.lzh"
        [IO.File]::WriteAllBytes($noEnd,[byte[]]$bytes[0..($bytes.Length-2)])
        Add-ExtractionHeaderFixture "$family-jm$method-h$level-no-end" $noEnd 0 3
    }}}
}
if('short' -in $Groups){
    $ShortHeaderDirectory=(Resolve-Path -LiteralPath $ShortHeaderDirectory).Path
    foreach($level in 0,1,2,3){
        $paths=@(Get-ChildItem -LiteralPath (Join-Path $ShortHeaderDirectory "h$level") -Filter '*.lzh' | Where-Object BaseName -ne 'seed' | Sort-Object Name)
        if($paths.Count -lt 7){throw '短いヘッダーの入力が不足しています'}
        foreach($path in $paths){
            $expected=if($path.BaseName -in 'good','badEnd','cut1'){0}else{32834}
            $members=if($path.BaseName -in 'good','badEnd'){3}else{2}
            Add-ExtractionHeaderFixture "short-h$level-$($path.BaseName)" $path.FullName $expected $members
        }
    }
}
if('initial' -in $Groups){
    $InitialHeaderDirectory=(Resolve-Path -LiteralPath $InitialHeaderDirectory).Path
    $paths=@(Get-ChildItem -LiteralPath $InitialHeaderDirectory -Filter '*.lzh' | Sort-Object Name)
    if($paths.Count -ne 30){throw '先頭ヘッダーの 30 入力が必要です'}
    foreach($path in $paths){
        $valid=$path.BaseName -eq 'valid' -or $path.BaseName.EndsWith('-valid')
        Add-ExtractionHeaderFixture "initial-$($path.BaseName)" $path.FullName $(if($valid){0}else{32795}) $(if($valid){1}else{0})
    }
}
if('missing-crc' -in $Groups){
    $MissingCrcDirectory=(Resolve-Path -LiteralPath $MissingCrcDirectory).Path
    foreach($name in 'missing-good','good-missing','good-missing-good','missing-pmarc-good'){
        Add-ExtractionHeaderFixture $name (Join-Path $MissingCrcDirectory "$name.lzh") $(if($name.StartsWith('missing-')){32795}else{32845}) -1
    }
}
if($FixtureNames.Count){
    foreach($name in $FixtureNames){if($name -notin $fixtures.Name){throw "未知の入力です: $name"}}
    $fixtures=@($fixtures | Where-Object Name -in $FixtureNames)
}
$hashes=@{}
$stamps=@{}
foreach($path in @($TestProgram,$runner,$oracle,$candidate)+@($fixtures.Path)){
    $hashes[$path]=(Get-FileHash -LiteralPath $path).Hash
    $stamps[$path]=[IO.File]::GetLastWriteTimeUtc($path)
}
Write-Host "Extraction header errors: candidate SHA256=$($hashes[$candidate]), oracle SHA256=$($hashes[$oracle])"
$observations=[Collections.Generic.List[object]]::new()
$expectedTotal=$fixtures.Count*$Commands.Count*$ProfileNames.Count*$NameModes.Count
foreach($fixture in $fixtures){foreach($command in $Commands){foreach($mode in $NameModes){foreach($profileName in $ProfileNames){
    $profile=$profiles[$profileName]
    $aborting=$profile.ContainsKey('Abort') -and $mode -ne 0
    $archive=$fixture.Path
    $label="$($fixture.Name)-$command-n$mode-$profileName"
    $snapshots=@()
    foreach($side in 'oracle','reimpl'){
        # 同じ長さの専用出力先を使い、各条件のファイル生成・上書きを領域内に閉じる。
        $root=Join-Path $workspace ('case-{0:D4}-{1}' -f $observations.Count,$side)
        $outputDirectory=Join-Path $root $DestinationName
        New-Item -ItemType Directory -Path $outputDirectory | Out-Null
        $destination=$outputDirectory.Replace('\','/')+'/'
        $dll=if($side -eq 'oracle'){$oracle}else{$candidate}
        $line="$command -+ -gm1 -y1 -n$mode `"$archive`" `"$destination`" `"$($profile.Pattern)`""
        $arguments=if($profile.Capacity){@('--command-raw-probe',$dll,$line,$profile.Api,[string]$profile.Capacity)}
            else{@('--command-enum-probe',$dll,$line,$profile.Layout,$profile.Selected,$profile.Replacement,'1041',[string]$UnicodeMode,$profile.Api,'1')}
        if($profile.Capacity -and $UnicodeMode){$arguments+=,'utf8'}
        if($profile.ContainsKey('Abort')){$arguments+=,[string]$profile.Abort}
        $rows=@(Invoke-EnumProbe (Join-Path $root 'command') $arguments)
        $expectedResult=if($aborting -and $fixture.Expected -ne 32795){32800}else{$fixture.Expected}
        if(@($rows -match '^result=').Count -ne 1 -or !@($rows -match "^result=$expectedResult(?:,|$)").Count){throw "戻り値が不正です: $label/$side"}
        if($profile.Capacity){
            $units=@((($rows[0] -split 'raw=',2)[1]).TrimEnd(',').Split(','))
            $guard=if($profile.Api -eq 'W'){'cccc'}else{'cc'}
            if($units.Count -ne $profile.Capacity+16 -or @($units[0..7] -cne $guard).Count -or @($units[($profile.Capacity+8)..($profile.Capacity+15)] -cne $guard).Count){throw '生バッファの境界が変更されました'}
        }
        # コールバックによる非表現文字の置換先も、この条件専用の領域全体で確認する。
        $files=@(Get-ChildItem -LiteralPath $root -Recurse -File | Where-Object DirectoryName -ne $root | Sort-Object FullName | ForEach-Object {
            $relative=[IO.Path]::GetRelativePath($root,$_.FullName).Replace('\','/')
            "file=$relative,size=$($_.Length),sha256=$((Get-FileHash -LiteralPath $_.FullName).Hash),attributes=$([int]$_.Attributes),write=$($_.LastWriteTimeUtc.Ticks)"
        })
        $expectedFiles=if($profile.Pattern -eq 'missing' -or $profileName -eq 'reject' -or $aborting){0}elseif($profileName -eq 'rename' -and $fixture.Members -gt 0){1}else{$fixture.Members}
        if($expectedFiles -ge 0 -and $files.Count -ne $expectedFiles){throw "展開ファイル数が不正です: $label/$side"}
        $directories=@(Get-ChildItem -LiteralPath $root -Recurse -Directory | Sort-Object FullName | ForEach-Object {
            "directory=$([IO.Path]::GetRelativePath($root,$_.FullName).Replace('\','/')),attributes=$([int]$_.Attributes)"
        })
        $effects=@($files)+@($directories)
        [IO.File]::WriteAllLines((Join-Path $root 'effects.txt'),[string[]]$effects)
        $rows+=$effects
        if((Get-FileHash -LiteralPath $archive).Hash -cne $hashes[$archive] -or [IO.File]::GetLastWriteTimeUtc($archive) -ne $stamps[$archive]){throw '元書庫が変更されました'}
        $snapshots+=,@($rows | ForEach-Object {$_.Replace($root.Replace('\','/'),'<ROOT>').Replace($root.Replace('\','\\'),'<ROOT>')})
    }
    $difference=@(Compare-Object $snapshots[0] $snapshots[1] -SyncWindow 0)
    if($difference.Count){$difference | Export-Csv -LiteralPath (Join-Path $workspace "$label-diff.tsv") -Delimiter "`t" -NoTypeInformation}
    $observations.Add([pscustomobject]@{Case=$observations.Count;Fixture=$fixture.Name;Command=$command;Mode=$mode;Profile=$profileName;Differences=$difference.Count})
}}}
    $observations | Export-Csv -LiteralPath (Join-Path $workspace 'observations.tsv') -Delimiter "`t" -NoTypeInformation
    Write-Host "$($fixture.Name) comparisons=$($observations.Count)"
}
foreach($path in $hashes.Keys){if((Get-FileHash -LiteralPath $path).Hash -cne $hashes[$path] -or [IO.File]::GetLastWriteTimeUtc($path) -ne $stamps[$path]){throw "検証中に入力またはバイナリーが変更されました: $path"}}
if(!$expectedTotal -or $observations.Count -ne $expectedTotal){throw '展開ヘッダー比較の件数が不足しています'}
$failures=@($observations | Where-Object Differences -ne 0)
if($failures.Count){$failures | Format-Table -AutoSize | Out-String -Width 200 | Write-Host; throw '展開ヘッダーのログ・通知・状態・出力ファイルが一致しません'}
Write-Host "Extraction header errors: $($observations.Count) exact comparisons, buffer and immutable-input guards passed"
