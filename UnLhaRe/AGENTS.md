...
- API level 2または.NET APIを変える場合は `src/operation.rs`、`src/ffi/app.rs`、`include/unlhare.h`、`tests/app_ffi.rs`、`tests/operation.rs`、`bindings/dotnet/src/`、同ContractTestsをまとめて照合する。Windowsではネイティブ対象のbundle作成後、`.github/workflows/modern.yml` と同じNative AOT publishと実行試験を行う。
...
WindowsでAPI level 2または.NET APIを変更した場合は、次を実行する。
