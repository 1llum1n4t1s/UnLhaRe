...
- JSON連携APIまたは.NET APIを変える場合は `src/operation.rs`、`src/ffi/app.rs`、`include/unlhare.h`、`tests/app_ffi.rs`、`tests/app_usability_ffi.rs`、`tests/operation.rs`、`bindings/dotnet/src/`、同ContractTestsをまとめて照合する。Windowsではネイティブ対象のbundle作成後、`.github/workflows/modern.yml` と同じNative AOT publishと実行試験を行う。
